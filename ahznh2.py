...
def simulate_sqlmap_hacking():
    ...
    for i in range(10):
        table.add_row(str(i+1), f"{i*88}a{i*44}I&trt6rTR{i*44}I&trt6r6TRUt6r^f*Tf8t{i}", f"t6rtrfutRITR{i*44}I&trt6rg{i*94}t*6trG{i*14}^r8RT{i}", f"t8{i*34}9T^ggt{i*34}t^G{i*34}T87RR^r&")
    ...
    while True:
        ...
        if command == "dump":
            console.print("[bold blue]DUMPING DATABASE...[/bold blue]")
            time.sleep(2)
            console.print("[bold red]TABLE, CONVERTING FROM 'JSON' TO 'XLSX'[/bold red]")
            
            console.print("[bold blue]Saved Table Instince to C:/.[/bold  blue]")
...
def main():
    console.print(pyfiglet.figlet_format("HACKING WxCX DATABASE"))
